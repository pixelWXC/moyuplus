...
- 复用 Reader Webview 增加阅读/快捷键双面板，不新增独立视图。
- 快捷键面板展示命令名称、默认绑定或“未设置默认按键”、启用状态、风险和动作说明；实际用户绑定交给 VS Code Keyboard Shortcuts 编辑器查看与修改，避免依赖非公开 API 或直接解析 `keybindings.json`。
- Enter/Tab 提供插件内启用开关，其余动作提供“编辑按键”入口；补齐阅读器翻页、切换文件、字号、打开与关闭等可绑定命令。
- 首次启动练习通过 global state 记录一次性安全提示。
- Reader 状态消息增加可操作错误类型；无文件可直接导入，文件失效可重新导入/移除，解码失败可切换 UTF-8/GBK。
- 按 TDD 分为快捷键目录与 package 契约、Provider/Webview 交互、安全提示、异常恢复四组 RED→GREEN 循环。

执行状态：

- 2026-07-10 已完成上述实现，并增加 `.impeccable.md` 固化“原生、克制、可靠”的界面设计基线。
- `npm test` 全量通过：10 个测试文件、57 个测试。
- `npm run compile` 和 `git diff --check` 通过。
- 2026-07-10 用户确认 Extension Development Host 人工测试通过：快捷键页、原生改键入口、安全提示与异常恢复操作均符合预期。
...
Phase 7 已完成自动验证和人工验证。下一步进入 Phase 8：测试、打包与人工验收。
