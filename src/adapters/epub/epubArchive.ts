...
export class EpubArchive {
  ...
  static async open(file: string, overrides: Partial<EpubSecurityPolicy> = {}): Promise<EpubArchive> {
    ...
    try { await new Promise<void>((resolve, reject) => { zip.on('error', reject); zip.on('end', resolve); zip.on('entry', (entry) => { try { count++; validateEntry(entry, count, total, policy); total += entry.uncompressedSize; index.set(normalizeArchivePath(entry.fileName), entry); zip.readEntry(); } catch (error) { reject(error); zip.close(); } }); zip.readEntry(); }); } catch (error) { zip.close(); throw error; }
...
export function normalizeArchivePath(value: string): string { const decoded = value.replace(/\\/g, '/'); if (!decoded || decoded.startsWith('/') || /^[a-z]:/i.test(decoded) || decoded.split('/').some((x) => x === '..' || x === '')) throw new EpubSecurityError(`Unsafe EPUB path: ${value}`); return decoded.replace(/^\.\//, ''); }
