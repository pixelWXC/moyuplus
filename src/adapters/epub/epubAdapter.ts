...
class EpubBookHandle implements BookHandle<EpubLocator> {
  ...
  async normalizeLocator(locator: ReadingLocator): Promise<EpubLocator> { this.assertOpen(); const existing = this.pkg.sections.find((x) => x.id === locator.sectionId); const section = existing ?? this.pkg.sections[0]; const normalized: EpubLocator = { kind: 'epub', sectionId: section.id, progression: existing ? normalizeProgression(locator.progression) : 0 }; if (locator.kind === 'epub' && existing) { if (locator.cfi) normalized.cfi = locator.cfi; if (locator.fragment) normalized.fragment = locator.fragment; if (locator.textOffset !== undefined) normalized.textOffset = locator.textOffset; if (locator.immersiveOffset !== undefined) normalized.immersiveOffset = locator.immersiveOffset; if (locator.sourceRevision) normalized.sourceRevision = locator.sourceRevision; if (locator.projectionRevision) normalized.projectionRevision = locator.projectionRevision; } return normalized; }
