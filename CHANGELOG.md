...
- 沉浸阅读不再过滤 EPUB 图片入口，改为使用与当前沉浸正文完全一致的 Decoration 字体、颜色、字重、斜体和背景样式显示“查看图片”文字。
- 图片入口悬浮操作复用普通阅读的安全图片预览，并校验当前书籍、章节和不透明资源 ID；翻页、切章或退出后的旧请求不会打开图片，打字练习素材也不会混入图片入口文字。
