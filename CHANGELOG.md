...
- 移除统一设置面板中遗留的“打字练习（实验性）”分区，以及全部 VS Code 扩展设置贡献；打字练习与 EPUB 阅读继续使用各自的插件内设置。
- 移除依赖扩展设置的兼容性 Enter 路由；原有打字练习字号、行高、字体和虚拟键盘显示设置迁入 MoyuPlus Typing 自己的设置页与插件存储，不再读取工作区或用户设置；虚拟键盘固定使用左右手区域配色。

## 0.3.0

- 打字练习侧栏默认隐藏无法独立打开的“设置”入口；选择有效素材后，改为在常驻导航末尾显示独占一行的“本次设置”，失效草稿会返回素材页并给出页内提示。
- 重设计打字练习“强化”页：历史错词按每批最多 20 词排队，一次完整正确即稳定，仍然出错则进入下一轮；新增一键开始、调整本次练习、空状态引导和最近一批结果反馈。
